...
class ChainfileResolverTest(unittest.TestCase):
    ...
    def test_that_it_does_not_work_with_invalid_data(self):
        result = ChainfileResolver.resolve_chainfile(os.path.join(self.configPath, 'invalid_data.yml'))
        assert result is None

    def test_that_it_does_not_work_on_non_yml_files(self):
        result = ChainfileResolver.resolve_chainfile(os.path.join(self.configPath, 'invalid_yml_file.yml'))
        assert result is None

    def test_that_it_does_not_work_if_path_is_not_file(self):
        result = ChainfileResolver.resolve_chainfile(self.configPath)
        assert result is None
    ...
    def test_that_it_only_adds_existing_data_to_model(self, file, expected):
        result = ChainfileResolver.resolve_chainfile(os.path.join(self.configPath, file))
